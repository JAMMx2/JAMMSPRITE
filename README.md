...
# print the first frame straight to your terminal
python asciisprite.py cat.png --name idle --print
```
...
| `<name>.gif` | animated preview (phosphor green by default, `--color` for full colour) |
...
| `--color` | off (green) | render GIF/sheet in full colour |
...
- To face the other way, mirror each row (reverse the string / the colour row).

Drop that into a `<pre>` element (monospace) or a canvas and step through the
frames — that's a sprite.
